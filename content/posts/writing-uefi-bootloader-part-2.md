...
Unfortunately this means that we can't use the native `str` and `String` types, and instead have to use the `uefi-rs` provided `CStr16` and `CString16` types.
...
One small difference to `std::fs` here is that, while both the `std` and `uefi` `read()`s take anything that can be borrowed as a `Path`, `CStr16` (unlike `str`) can't be implicitly borrowed as a `Path`.
